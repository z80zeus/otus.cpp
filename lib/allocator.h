 ...
 * |-----|-----|-----|      |-----|
 *
 * Блоки данных выделяются начиная из головы списка:
 *
 * |-----|-----|-----|      |-----|
 * |<-T->|<-T->|  T* -> ... -> 0  |
 * |-----|-----|-----|      |-----|
 *
 * Возвращаются блоки также в голову списка:
 *      --------
 * |-- /-|-----|-\---|      |-----|
 * |  T* |<-T->|  T* -> ... -> 0  |
 * |-----|-----|-----|      |-----|
 ...
 * При освобождении ранее выделенных блоков - они будут возвращаться в голову нового списка.
 * Разумеется, освобождение блоков приведёт к фрагментации, а поиск блока достаточной длины во фрагментированном списке
 * это операция, требующая линейного времени, но - это всего лишь учебный пример ¯\_(ツ)_/¯
 *
 * Кроме того, такой подход накладывает ограничение: организация хранилища блоков в виде связного списка прямо поверх
 * массива данных, требует, чтобы размер объекта типа Т, с которым работает аллокатор - был не меньше размера указателя.
*/
#pragma once
